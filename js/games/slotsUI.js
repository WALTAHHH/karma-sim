...
function bindEvents(onClose) {
    ...
    document.getElementById('debug-add-karma')?.addEventListener('click', () => {
        currentKarma += 50;
        addFn(50);
        updateKarmaDisplayLocal(currentKarma, 50);
        updateButtonStates();
    });
}
...
async function handleMultiPull() {
    ...
    updateKarmaDisplayLocal(currentKarma, -getMultiPullCost());
...
async function showPullResult(reward, rarity) {
    ...
    if (reward.rarity === 'legendary') {
        ...
        shakeScreen(document.querySelector('.gacha-machine'));
...
async function showMultiPullResults(results) {
    ...
    if (totalKarmaGain > 0) {
        ...
        updateKarmaDisplayLocal(currentKarma, totalKarmaGain);
    }
...
// Local wrapper for karma display update (uses slots-specific element IDs)
